...
(function() {
  ...
  add_immediate_actions = function(control, prefix, data, meldload) {
    var channel, l, len2, len3, len4, m, msg, msgs, nextstage, nextstages, ns, o, q, ref11, ref12, ref13, sfbi, sfi, stageflags, stagetest, t, text;
    control.actions.push({
      channel: '',
    ...
    if (data[prefix + 'cue'] != null) {
      ...
      for (o = 0, len4 = nextstages.length; o < len4; o++) {
        nextstage = nextstages[o];
  ...
  for (r = q = 1; q <= 1000; r = ++q) {
    ...
    codetitles = {};
    for (mi = u = 0, len3 = mcs.length; u < len3; mi = ++u) {
      mc = mcs[mi];
      if (!data[mc + 'name']) {
        continue;
      }
      ...
      if (codetitles[marker.title] === void 0) {
        ...
        for (x = 0, len4 = ref13.length; x < len4; x++) {
          ct = ref13[x];
      ...
      if ((data[mc] != null) && data[mc] !== '') {
        ...
        fragments = [];
        for (y = 0, len5 = labels.length; y < len5; y++) {
          label = labels[y];
          if (label !== '') {
            if ((label.indexOf('#')) === 0) {
              ...
              if (ids == null) {
                ...
                for (z = 0, len6 = ids.length; z < len6; z++) {
                  id = ids[z];
                  fragments.push('#' + id);
                }
              }
        ...
        for (i1 = 0, len7 = fragments.length; i1 < len7; i1++) {
          fragment = fragments[i1];
          marker.actions.push({
            url: '{{meldcollection}}',
            post: true,
            contentType: 'application/json',
  ...
  for (stage in stages) {
    data = stages[stage];
    for (j1 = 0, len8 = prefixes.length; j1 < len8; j1++) {
      prefix = prefixes[j1];
      if (data[prefix + 'cue'] != null) {
        nextstages = ((data[prefix + 'cue'].split('/')).map((function(_this) {
          return function(s) {
        ...
        for (k1 = 0, len9 = nextstages.length; k1 < len9; k1++) {
          nextstage = nextstages[k1];
  ...
  for (sfi = l1 = 1, ref14 = numflagvars; 1 <= ref14 ? l1 <= ref14 : l1 >= ref14; sfi = 1 <= ref14 ? ++l1 : --l1) {
  ...
  for (m1 = 0, len10 = ref15.length; m1 < len10; m1++) {
    channel = ref15[m1];
  ...
  ref16 = ex.markers;
  for (n1 = 0, len11 = ref16.length; n1 < len11; n1++) {
    marker = ref16[n1];
