...
(function() {
  var BITS_PER_FLAGVAR, add_actions, add_delayed_actions, add_immediate_actions, cell, cellid, climbview, codetitles, config, configdir, configfile, content_url, control, ct, cuesingle, data, defaultprojection, delayid, e, effects, errors, ex, exinfile, exoutfile, fragment, fragments, fs, getCodeIds, get_marker, guid, i, id, ids, j, k, l, label, labels, len, len1, len2, len3, len4, len5, len6, len7, len8, m, marker, markernames, maxdelay, maxrow, mc, mcs, meiids, mi, mindelay, name, numflagvars, numstages, o, p, path, prefix, prefixes, q, r, readmeiids, readrow, ref, ref1, ref10, ref11, ref12, ref13, ref14, ref2, ref3, ref4, ref5, ref6, ref7, ref8, ref9, relpath, s, set_stage, sfi, sheet, stage, stages, sweathers, t, title, u, v, viewconfig, viewgen, viewoutfile, w, weather_urls, weatherdelay, weathers, wi, workbook, x, xlfile, xlsx, y, yaml, z;
  ...
  add_actions = function(control, prefix, data, meldload) {
    ...
    if (data[prefix + 'delay'] != null) {
      try {
        ...
        err = error;
        return console.log('ERROR: adding delay of ' + data[prefix + 'delay'] + ' for ' + data.stage + ' ' + prefix);
      }
    } else {
      return add_delayed_actions(control, prefix, data, meldload);
    }
  };

  add_immediate_actions = function(control, prefix, data, meldload) {
    var altstage, channel, k, l, len2, len3, m, meldprefix, msg, msgs, nextexp, nextstage, nexturi, ns, o, ref11, ref12, ref13, sfbi, sfi, stageflags, stagetest;
    control.actions.push({
      channel: '',
    ...
    if (data[prefix + 'cue'] != null) {
      ...
      if ((nextstage != null) && (data.next != null) && (stages[nextstage] != null) && (stages[data.next] != null) && nextstage !== data.next) {
        stageflags = [];
        for (sfi = m = 1, ref12 = numflagvars; 1 <= ref12 ? m <= ref12 : m >= ref12; sfi = 1 <= ref12 ? ++m : --m) {
          stageflags[sfi - 1] = 0;
        }
        ns = nextstage;
        ...
        altstage = data.next;
        stagetest = '';
        for (sfi = o = 1, ref13 = numflagvars; 1 <= ref13 ? o <= ref13 : o >= ref13; sfi = 1 <= ref13 ? ++o : --o) {
  ...
  add_delayed_actions = function(control, prefix, data, meldload) {
    var channel, k, l, len2, len3, msg, msgs, ref11, results;
    ref11 = ['v.animate'];
    for (k = 0, len2 = ref11.length; k < len2; k++) {
      channel = ref11[k];
      if (data[prefix + channel] != null) {
  ...
  set_stage = function(control, data) {
    var k, l, len2, n, ni, ref11, sfbi, sfi, ws;
    if (control.poststate == null) {
      control.poststate = {};
    }
  ...
  for (sfi = m = 1, ref11 = numflagvars; 1 <= ref11 ? m <= ref11 : m >= ref11; sfi = 1 <= ref11 ? ++m : --m) {
  ...
  for (r = o = 1; o <= 1000; r = ++o) {
    ...
    codetitles = {};
    for (mi = s = 0, len3 = mcs.length; s < len3; mi = ++s) {
      mc = mcs[mi];
      if (!data[mc + 'name']) {
        continue;
      }
      ...
      if (codetitles[marker.title] === void 0) {
        ...
        for (t = 0, len4 = ref13.length; t < len4; t++) {
          ct = ref13[t];
      ...
      if ((data[mc] != null) && data[mc] !== '') {
        ...
        fragments = [];
        for (u = 0, len5 = labels.length; u < len5; u++) {
          label = labels[u];
          if (label !== '') {
            if ((label.indexOf('#')) === 0) {
              ...
              if (ids == null) {
                ...
                for (v = 0, len6 = ids.length; v < len6; v++) {
                  id = ids[v];
                  fragments.push('#' + id);
                }
              }
        ...
        for (x = 0, len7 = fragments.length; x < len7; x++) {
  ...
  for (stage in stages) {
    data = stages[stage];
    for (y = 0, len8 = prefixes.length; y < len8; y++) {
      prefix = prefixes[y];
  ...
  for (sfi = z = 1, ref14 = numflagvars; 1 <= ref14 ? z <= ref14 : z >= ref14; sfi = 1 <= ref14 ? ++z : --z) {
