...
(function() {
  ...
  add_stage_announcement = function(control, stage, next) {
    return control.actions.push({
      channel: 'stageview',
      url: 'data:text/html,<h1>Now: <b>(' + (stage != null ? (stage._index + 1) + ')</b> ' + stage.stage : '{{stageix}}) {{stage}}') + ',<br>Next: (' + (next != null ? (next._index + 1) + ') ' + next.stage : '{{nextstageix}}) {{nextstage}}') + '</h1>'
    });
  };
  ...
  for (r = t = 1; t <= 1000; r = ++t) {
    ...
    add_stage_announcement(control, {
      stage: ' ',
      _index: 0
    });
    ex.controls.push(control);
